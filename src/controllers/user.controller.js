...
const registerUser = asyncHandler( async(req, res) => {
     ...
     // check for images, chack for avtar
...
const changeCurrentPassword = asyncHandler(async(req, res) => {
    ...
    if(!(newPassword === confPassword)){
        throw new ApiError (400, "Enter the confPassword");
    }
...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {
                    $cond: {
                        if: {$in: [req.user?._id, "$subscribers.subscrber"]},
                        then: true,
                        false: false
                    }
                }
    ...
    .status(200)
    .json(
        new ApiResponse(200, channel[0],"Uaer channel feached successfully")
    )
})
...
const getWatchHistory = asyncHandler(async(req, res) => {
    ...
    .status(200)
    .json(
        new ApiResponse(200, user[0].watchHistory, "watch historu fetched successfully")
    )
})
